...
def _correct_to_print(file_obj):
    ...
    A file refused by the printable adapter keeps its None value: None is not "off",
    it means printing is impossible, and the icon must stay blocked.
    """
    if getattr(file_obj, "to_print", False) is None:
        return
