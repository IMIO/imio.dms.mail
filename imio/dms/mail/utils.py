...
class VariousUtilsMethods(UtilsMethods):
    ...
    def duplicate_folder(self, keep_classification_categories, keep_treating_groups, keep_recipient_groups, keep_subfolders):
        original_folder = self.context
        if not IClassificationFolder.providedBy(original_folder):
        ...
        # Keep classification_categories
        if keep_classification_categories and original_folder.classification_categories:
            duplicated_folder.classification_categories = original_folder.classification_categories[:]
        else:
            duplicated_folder.classification_categories = None

        # Keep treating_groups
        if keep_treating_groups and original_folder.treating_groups:
            duplicated_folder.treating_groups = original_folder.treating_groups[:]
        else:
            duplicated_folder.treating_groups = None

        # Keep recipient_groups
        if keep_recipient_groups and original_folder.recipient_groups:
            duplicated_folder.recipient_groups = original_folder.recipient_groups[:]
        else:
