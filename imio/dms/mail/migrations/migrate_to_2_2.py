...
class Migrate_To_2_2(Migrator):
    ...
    def update_site(self):
        ...
        if (
            "indispo" in self.portal["messages-config"]
            and api.content.get_state(self.portal["messages-config"]["indispo"]) == "activated"
        ):
    ...
    def check_roles(self):
        ...
        for group in api.group.get_groups():
            ...
            for role in roles:
                if (
                    role == "Authenticated"
                    or (role == "Manager" and group.id == "Administrators")
                    or (role == "Site Administrator" and group.id == "Site Administrators")
                    or (role == "Reviewer" and group.id == "Reviewers")
                ):
                    continue
                else:
    ...
    def run(self):
        ...
        self.runProfileSteps(
            "imio.dms.mail",
            steps=["actions", "browserlayer", "componentregistry", "plone.app.registry", "portlets", "viewlets"],
        )
