...
class Migrate_To_1_1(Migrator):
    ...
    def add_view_field(self, fldname, folder, ids=[], before=""):
        ...
        crit = {"portal_type": "DashboardCollection", "path": {"query": "/".join(folder.getPhysicalPath()), "depth": 1}}
        if ids:
            crit["id"] = ids
    ...
    def run(self):
        ...
        self.runProfileSteps(
            "collective.messagesviewlet", steps=["collective-messagesviewlet-messages"], profile="messages"
        )
        ...
        self.update_count(
            im_folder["mail-searches"], ids=["to_validate", "to_treat", "im_treating", "searchfor_created"]
        )
