...
class Migrate_To_2_3(Migrator):  # noqa
    ...
    def run(self):
        ...
        if "wf_from_to" not in get_dms_config():
            ...
            set_dms_config(
                ["wf_from_to", "dmsoutgoingmail", "n_plus", "to"], [("to_be_signed", "propose_to_be_signed")]
            )
    ...
    def update_site(self):
        ...
        for state in ["proposed_to_manager", "proposed_to_n_plus_1", "proposed_to_agent", "in_treatment", "closed"]:
            if state in lrsc:
    ...
    def remove_service_chief(self):
        ...
        for attr in ("imail_remark_states", "omail_remark_states"):
            lst = (
                api.portal.get_registry_record("imio.dms.mail.browser.settings.IImioDmsMailConfig.{}".format(attr))
                or []
            )
            if "proposed_to_service_chief" in lst:
                lst.remove("proposed_to_service_chief")
                api.portal.set_registry_record("imio.dms.mail.browser.settings.IImioDmsMailConfig.{}".format(attr), lst)
            ...
            if u"imio.dms.mail.wfadaptations.{}SkipProposeToServiceChief".format(acr) in applied_wfa:
                ...
                if adapt_is_applied:
                    add_applied_adaptation(
                        "imio.dms.mail.wfadaptations.{}ServiceValidation".format(acr), wkf, True, **n_plus_1_params
                    )
        ...
        if u"imio.dms.mail.wfadaptations.IMPreManagerValidation" in applied_wfa:
            ...
            params = [
                ...
                if dic["adaptation"] == u"imio.dms.mail.wfadaptations.IMPreManagerValidation"
            ][0]
            ...
            if adapt_is_applied:
                add_applied_adaptation(
                    "imio.dms.mail.wfadaptations.IMPreManagerValidation", "incoming_mail", False, **params
                )
        ...
        config = {
            "dmsincomingmail": {
                ...
                "tr": {"propose_to_service_chief": "propose_to_n_plus_1", "back_to_service_chief": "back_to_n_plus_1"},
            },
            "dmsoutgoingmail": {
