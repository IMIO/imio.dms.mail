...
class TestBrowserTask(unittest.TestCase, ImioTestHelpers):
    ...
    def test_AssignedUsersVocabulary(self):
        ...
        self.assertListEqual(sorted([t.value for t in voc_inst(None)]),
                             ['agent', 'agent1', 'chef', 'dirg', 'encodeur', 'lecteur', 'scanner', 'siteadmin',
                              'test_user_1_'])
        self.assertListEqual([t.title for t in voc_inst(None)],
                             [u'Fred Agent', u'Jean Encodeur', u'Jef Lecteur', u'Maxime DG', u'Michel Chef',
                              u'siteadmin', u'Scanner', u'Stef Agent', u'test_user_1_'])
