...
class TestDocumentGenerator(unittest.TestCase):
    ...
    def test_DocumentGenerationOMDashboardHelper(self):
        ...
        # Test get_dms_files: returns every file (main + appendix) marked "to_print",
        # main files before appendix files, across the selected mails (PARAF-220).
        ...
        # Test is_odt (works on both dmsommainfile and dmsappendixfile)
        self.assertTrue(view.is_odt(m0["1"]))
        self.assertFalse(view.is_odt(appendix))

        # add an image appendix (a logo) to check the native-size image branch (PARAF-220)
        ...
        # Test is_image: True only for image files, not odt main nor pdf appendix
        self.assertTrue(view.is_image(img_appendix))
        self.assertFalse(view.is_image(m0["1"]))
        self.assertFalse(view.is_image(appendix))
        # Test img_format: mimetype-derived appy/pod image format (jpeg -> jpg)
        ...
        # Test get_print_pages: [] for odt and image files, preview page images otherwise
        ...
        # Test print_page_count / needs_blank_after (PARAF-220 duplex blank page)
        # an image occupies a single page (odd) -> always needs a trailing blank page
        self.assertEqual(view.print_page_count(img_appendix), 1)
        self.assertTrue(view.needs_blank_after(img_appendix))
        # a non-odt file occupies its documentviewer preview pages -> blank iff odd
