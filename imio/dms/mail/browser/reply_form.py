...
class MultipleReplyForm(BaseReplyForm):
    ...
    def label(self):
        return _(
            u"Reply to ${ref}",
            mapping={"ref": u"%d %s" % (len(self.brains), api.portal.translate("incoming mails", "imio.dms.mail"))},
        )
    ...
    def updateWidgets(self):
        ...
        if self.uids:  # see upper comment
            ...
            self.widgets["IDublinCore.title"].value = u"%s%s" % (
                prefix,
                safe_unicode(first and self.brains[0].get_full_title or ""),
            )
            ...
            self.widgets["recipient_groups"].value = list(
                set([uid for b in self.brains if b.recipient_groups for uid in b.recipient_groups])
            )
        imio_dmsoutgoingmail_updatewidgets(self)
