...
class DmsOMActionsPanelView(ActionsPanelView):
    ...
    transitions = ['back_to_agent', 'back_to_creation', 'back_to_n_plus_1', 'back_to_validated', 'back_to_be_signed',
                   'back_to_scanned', 'propose_to_n_plus_1', 'set_validated', 'propose_to_be_signed',
    ...
    def may_create_from_template(self):
        ...
        """
        if not self.isInFacetedNavigation() and self.member.has_permission('collective.dms.basecontent: Add DmsFile',
                                                                           self.context):
            return True
        return False
