...
class DocumentGenerationOMDashboardHelper(DocumentGenerationDocsDashboardHelper):
    ...
    def get_dms_files(self, limit=None):
        ...
        (dmsommainfile and dmsappendixfile) whose `to_print` attribute is True,
        main files first then appendix files, each group kept in folder position
        order. `limit`, when given, caps the total number of returned files.
        """
        files = []
        ...
        for brain in self.context_var("brains"):
            ...
            # main files (dmsommainfile) first, then appendix files; the sort is
            # stable so folder position order is preserved within each group
    ...
    def is_odt(self, afile):
        """Return True if the given file object is an ODT (used by the d-print template).

        Defined here because dmsappendixfile (collective.dms.basecontent) has no is_odt()
        method, unlike dmsommainfile (ImioDmsFile).
        """
    ...
    def is_image(self, afile):
        ...
        Such files are embedded at their native size (only shrunk if larger than the
        page) rather than as documentviewer previews, so a small image (e.g. a logo
        appendix) is not blown up to full page.
        """
    ...
    def print_page_count(self, afile):
        ...
        Image files are embedded one per page (always 1); other files (PDF, scans...)
        are inserted as their collective.documentviewer preview pages.
        """
    ...
    def get_print_pages(self, afile):
        ...
        pages = []
        for page in range(1, annot["num_pages"] + 1):
            blob = files["large/dump_%d.%s" % (page, fmt)]
            pages.append({"data": blob.open().read(), "format": fmt})
        return pages
