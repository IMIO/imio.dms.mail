...
def creating_group_filter_default(context):
    ...
    orgs = organizations_with_suffixes(
        get_plone_groups_for_user(user=current_user), [CREATING_GROUP_SUFFIX, CONTACTS_PART_SUFFIX], group_as_str=True
    )
    for term in voc:
...
class DmsContactSource(ContactSource):
    ...
    def __init__(
        self, context, selectable_filter, navigation_tree_query=None, default=None, defaultFactory=None, **kw
    ):  # noqa
        """Changes sort_on criteria from list to string."""
        super(DmsContactSource, self).__init__(
...
class ImioDmsIncomingMail(DmsIncomingMail):
    ...
    def is_n_plus_level_obsolete(self, treating_group="", state=None, config=None, state_start="proposed_to_n_plus"):
        ...
        return is_n_plus_level_obsolete(
            self, "dmsincomingmail", treating_group=treating_group, state=state, config=config, state_start=state_start
        )
...
class ImioDmsIncomingMailWfConditionsAdapter(object):
    ...
    def can_close(self):
        ...
        return is_in_user_groups(
            groups=("dir_general",),
            admin=True,
            suffixes=IM_EDITOR_SERVICE_FUNCTIONS,
...
class IMEdit(DmsDocumentEdit):
    ...
    def updateWidgets(self, prefix=None):
        ...
        if not sm.checkPermission("imio.dms.mail: Write mail base fields", self.context):
            ...
            if not getattr(self.context, "_iem_agent", False):
                if IDublinCore.__identifier__ in behaviors:
                    display_fields = ["IDublinCore.title", "IDublinCore.description"]
                elif IBasic.__identifier__ in behaviors:
                    display_fields = ["IBasic.title", "IBasic.description"]
        ...
        if not sm.checkPermission("imio.dms.mail: Write treating group field", self.context) and not getattr(
            self.context, "_iem_agent", False
        ):
        ...
        if (
            state == "created"
            and "ITask.due_date" in self.fields
            and self.widgets["ITask.due_date"].value == ("", "", "")
        ):
...
class ImioDmsOutgoingMail(DmsOutgoingMail):
    ...
    def is_n_plus_level_obsolete(self, treating_group="", state=None, config=None, state_start="proposed_to_n_plus"):
        """Check if current treating_groups has validators on the state"""
        return is_n_plus_level_obsolete(
            self, "dmsoutgoingmail", treating_group=treating_group, state=state, config=config, state_start=state_start
        )
    ...
    def copy_dms_files(self, original_mail):
        ...
        if dms_files and not self.mail_date:
            ...
            api.portal.show_message(
                translate('The mail date was missing and has been updating following the source mail.', domain='imio.dms.mail', context=self.REQUEST),
...
class ImioDmsOutgoingMailWfConditionsAdapter(object):
    ...
    def can_do_transition(self, transition):
        ...
        if (
            self.context.treating_groups in transitions_levels[state]
            and transitions_levels[state][self.context.treating_groups][way_index] == transition
        ):
...
def imio_dmsoutgoingmail_updatewidgets(the_form):
    ...
    if (the_form.context.portal_type != "dmsoutgoingmail" or not the_form.context.sender) and not the_form.widgets[
        "sender"
    ].value:
...
def manage_email_fields(the_form, action):
    ...
    if action == "edit":
        ...
        if not (
            the_form.context.email_subject
            or "edit-email" in the_form.request
            or the_form.request.form.get("form.widgets.email_subject")
        ):
            the_form.groups = [gr for gr in the_form.groups if gr.__name__ != "email"]
        return
...
class AssignedUserValidator(validator.SimpleFieldValidator):
    def validate(self, value, force=False):
        config = (
            (IMEdit, {"attr": "treating_groups", "schema": "", "fcts": IM_EDITOR_SERVICE_FUNCTIONS}),
        ...
        for klass, dic in config:
            if isinstance(self.view, klass):
                ...
                if (
                    getattr(self.context, dic["attr"]) is not None
                    and self.request.form.get(form_widget, False)
                    and self.request.form[form_widget][0] != getattr(self.context, dic["attr"])
                ):
                    # check if assigned_user is no more in
                    if (
                        self.context.assigned_user is not None
                        and value is not None
                        and value
                        not in get_selected_org_suffix_principal_ids(self.request.form[form_widget][0], dic["fcts"])
                    ):
                    ...
                    while doit:
                        doit, state, config = is_n_plus_level_obsolete(
                            self.context, "dmsincomingmail", treating_group=ntg, state=state, config=config
                        )
                        if doit:
                    ...
                    if transitions:
                        ...
                        for transition in transitions:
                            if not auc_config[transition].get(ntg, False):
                                raise Invalid(
                                    _(
                                        u"You must select an assigned user because the treating group "
